...
from USER_DIR import dense_image_warp
...
def _get_grid_locations(image_height, image_width):
    ...
    y_range = tf.linspace(0., tf.to_float(image_height) - 1., image_height)
    x_range = tf.linspace(0., tf.to_float(image_width) - 1., image_width)
...
def _expand_to_minibatch(tensor, batch_size):
    """Tile arbitrarily-sized tensor to include new batch dimension."""
    tiles = [batch_size] + [1] * tensor.get_shape().ndims
